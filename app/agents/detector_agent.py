"""Detector Agent — decides ONE smell/pattern type per call (multi-detector).

Generic by design: the type name/definition is injected into the per-call prompt
(see ``app/core/prompts.py``), not into static instructions. One agent instance
is reused across the 8 individual calls of phase 3 (4 smells + 4 patterns).
...
from app.core.prompts import TYPE_DETECTOR_INSTRUCTIONS
from app.core.schemas import TypeDetectionResult
